...
fire = pygame.mixer.Sound(os.path.join("Assets/Audio", "pop.ogg"))
...
class Enemy:
    ...
    def off_screen(self):
        return not(-50 <= self.x <= win_width + 50)
...
def draw_game():
    ...
    if not player.alive:
        ...
        text = font.render('You Died! Press X to restart', True, (255, 255, 255))
    ...
    text = font.render('Lives: '+ str(player.lives) + "  Score: " + str(kills*50), True, (0,0,0))
...
while run:
    ...
    if len(enemies) == 0:
        ...
        if rand_nr == 1:
            ...
            enemy = Enemy(1500, 700, left)
            enemies.append(enemy)
        if rand_nr == 0:
                # Spawn enemy on the right
            enemy = Enemy(50, 700, right)
            enemies.append(enemy)
    for enemy in enemies:
        enemy.move()
